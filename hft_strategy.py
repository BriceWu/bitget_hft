...
class HFTStrategy(ZMBase):
    ...
    async def start_hft(self):
        ...
        while True:
            try:
                ...
                if self._bn_price_changed and (self._have_placed_order == 0.) and (self.v_volume_rate.value >1.) and len(self._bb_price_list) > 8:
                    if (0.25 > self.v_trade_side.value > 0.) and (self._bn_ask_one / self._bitget_bid_one < self._sell_profit_rate * 0.9999):
                        ...
                        self._logger.info(f"开空 BN ask:{self._bn_ask_one}, bid:{self._bn_bid_one}, Bitget ask:{self._bitget_ask_one}, bid:{self._bitget_bid_one}")
                        self.send_wechat(self._mail_to, "HFT开空", self._bitget_bid_one)
                        await self.cancel_client_order()
                    elif (self.v_trade_side.value > 0.75) and (self._bn_bid_one / self._bitget_ask_one > self._buy_profit_rate * 1.0001):
                        self._have_placed_order = last_time
                        ...
                        self._logger.info(f"开多 BN ask:{self._bn_ask_one}, bid:{self._bn_bid_one}, Bitget ask:{self._bitget_ask_one}, bid:{self._bitget_bid_one}")
    ...
    async def close_position(self):
        ...
        if self._have_placed_order == 0.:
            return # 没有下单, 没有仓位
        if time.time() - self._have_placed_order < 30:  # 8s
            return
